// Note that `struct #name([u64; #limbs])` is stored in little-endian format, i.e. `limbs[0]` is the
// least significant limb. The endianness of `#name`'s limbs does not depend on the supplied
// `PrimeFieldReprEndianness` field in the `PrimeField` proc-macro. `ReprEndianness` only sets the
// endianess of the field element bytes representation `#name::Repr`.
...
// Used to implement optimized prime field arithmetic in x86-64 asm for BLS12-381's scalar field.
const BLS12_381_FR_MODULUS: &str =
...
impl ReprEndianness {
    ...
    /// Defines the implementation for `PrimeField::from_repr()` for the structure which
    /// `PrimeField` is being derived (`#name`).
    ...
    /// Defines the implementation of `PrimeField::to_repr()` for the structure which `PrimeField`
    /// is being derived (`#name`). Note that the `quote`-ed `self` and `r` refer to an instance of
    /// the prime field structure `#name`, not an instance of `ReprEndianness`.
    fn to_repr(
        &self,
...
fn prime_field_impl(
    ...
    fn mul_impl(
        ...
        if modulus.to_string() == BLS12_381_FR_MODULUS {
            assert_eq!(limbs, 4, "BLS12-381 scalar field must have 4 limbs");
            mul_impl_bls12_381_scalar_field(a, b)
        } else {
            mul_impl_default(a, b, limbs)
        }
    }

    /// If we are implementing prime field multiplication for BLS12-381's scalar field, we may be
    /// able to use the optimized x86-64 asm 4-limb implementation.
    fn mul_impl_bls12_381_scalar_field(
        a: proc_macro2::TokenStream,
        b: proc_macro2::TokenStream,
    ) -> proc_macro2::TokenStream {
        ...
        quote! {
            if cfg!(target_arch = "x86_64") && is_x86_feature_detected!("adx") {
                ::fff::mod_mul_4w_assign(&mut #a.0, &#b.0);
            } else {
                #default_impl
            }
        }
    ...
    fn add_assign_impl(
        ...
        if limbs == 4 {
            add_assign_4_limb_asm_impl(a, b)
        } else {
            add_assign_default_impl(a, b)
        }
    }

    // If we are compiling for x86-64 and the field has exactly 4 limbs, implement field addition in
    // asm.
    fn add_assign_4_limb_asm_impl(
        a: proc_macro2::TokenStream,
        b: proc_macro2::TokenStream,
    ) -> proc_macro2::TokenStream {
        let default_impl = add_assign_default_impl(a.clone(), b.clone());

        quote! {
            // Neither the asm or default implementations will exceed the backing capacity.
    ...
    // Implements field addition if we are not compiling for x86-64 or the field is not exactly 4
    // limbs.
    fn add_assign_default_impl(
        ...
        quote! {
            // It is not possible for `add_nocarry` to exceed the backing capacity, however its
            // output needs to be reduced.
            #a.add_nocarry(&#b);
            #a.reduce();
        }
    }
    ...
    let multiply_impl = mul_impl(quote! {self}, quote! {other}, limbs, modulus);
    let add_assign_impl = add_assign_impl(quote! {self}, quote! {other}, limbs);
    ...
    quote! {
        ...
        impl<'de> ::serde::Deserialize<'de> for #name {
            fn deserialize<D: ::serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
                ...
                #name::from_repr(repr).ok_or(D::Error::custom(format!(
                    "deserialized repr bytes encode an invalid field element (`{}`)",
                    #name,
                )))
            }
        }
        ...
        impl ::ff::Field for #name {
            ...
            fn double(&self) -> Self {
                ...
                // This will not exceed the backing capacity.
                let mut last = 0;
                for i in &mut ret.0 {
                    let tmp = *i >> 63;
                    *i <<= 1;
